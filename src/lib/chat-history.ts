...
export class ChatHistoryAPI {
  ...
  async getSessions(limit: number = 50, offset: number = 0): Promise<ChatSession[]> {
    // Use sessionsList timeout for consistent behavior with getSessionsWithCache retry wrappers
    const result = await this.makeRequest<ChatSession[]>(
      'GET',
      `/sessions?limit=${limit}&offset=${offset}`,
      null,
      TIMEOUT_CONFIG.chat.sessionsList // 10 seconds - matches getSessionsWithCache timeout
    );
    return result.data || [];
  }
  ...
  async getSessionsWithCache(limit: number = 50, offset: number = 0): Promise<ChatSession[]> {
    ...
    const sessions = await withTimeoutAndRetry(
      ...
      {
        timeout: TIMEOUT_CONFIG.chat.sessionsList, // 10 seconds - wrapper timeout for session fetch
        maxRetries: 3,
        shouldRetry: (error) => {
          if (error instanceof Error) {
            return error.name === 'AbortError' ||
