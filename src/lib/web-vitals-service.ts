...
class WebVitalsService {
  ...
  private generateSessionId(): string {
    // Check for crypto.randomUUID support (not available in older browsers like Twitter browser on Android 9)
    if (
    ...
    // Fallback: generate a UUID-like string using available crypto methods
    if (
      typeof window !== 'undefined' &&
      typeof crypto !== 'undefined' &&
