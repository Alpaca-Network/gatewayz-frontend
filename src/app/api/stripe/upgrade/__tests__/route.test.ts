...
describe('POST /api/stripe/upgrade', () => {
  ...
  it('should return 503 if Stripe is not configured', async () => {
    ...
    const request = new NextRequest('http://localhost/api/stripe/upgrade', {
      method: 'POST',
  ...
  it('should return 404 if no active subscription found', async () => {
    ...
    expect(response.status).toBe(404);
    expect(data.error).toBe('No active subscription found. Please subscribe first.');
  });
  ...
  it('should upgrade subscription successfully', async () => {
    ...
    expect(data.message).toContain('max');
    expect(mockSubscriptionsUpdate).toHaveBeenCalledWith('sub_123', {
      items: [{ id: 'si_123', price: 'price_max_123' }],
