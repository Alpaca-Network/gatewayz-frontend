...
async function safeParseJson(response: Response): Promise<{ success: boolean; data?: any; error?: string }> {
  ...
  if (contentType.includes('text/html')) {
    ...
    console.error('[Ranking Apps API] Backend returned HTML instead of JSON:', text.substring(0, 200));
    return { 
      success: false, 
  ...
  if (!contentType.includes('application/json')) {
    const text = await response.text();
    console.error('[Ranking Apps API] Unexpected content-type:', contentType, 'Body:', text.substring(0, 200));
    return { 
      success: false, 
  ...
  try {
    ...
    console.error('[Ranking Apps API] JSON parse error:', parseError);
    return { 
      success: false, 
