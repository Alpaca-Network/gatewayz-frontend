            ...
            {Object.keys(groupedSessions).length === 0 ? (
                ...
                Object.entries(groupedSessions).map(([groupName, chatSessions]) => (
                    <div key={groupName} className="min-w-0">
                        <h3 className="text-xs font-semibold text-muted-foreground uppercase my-1.5 px-2 sticky top-0 bg-background/95 backdrop-blur-sm z-10 py-1">{groupName}</h3>
...
function ChatPageContent() {
  ...
  return (
    <div className="flex h-screen max-h-[calc(100dvh-130px)] has-onboarding-banner:max-h-[calc(100dvh-210px)] bg-background overflow-hidden">
                      ...
                      {msg.role === 'user' ? (
                        ...
                        <div className="rounded-lg p-2.5 sm:p-3 bg-muted/30 dark:bg-muted/20 border border-border max-w-full w-full">
                          <div className="flex items-center justify-between mb-2">
                            {selectedModel?.label && <p className="text-xs font-semibold text-muted-foreground truncate">{selectedModel.label}</p>}
                          </div>
