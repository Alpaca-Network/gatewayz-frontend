...
var _chunkRGAQJSYNcjs = require('./chunk-RGAQJSYN.cjs');
...
var _chunkJUWXY2C3cjs = require('./chunk-JUWXY2C3.cjs');
...
async function serialize2(flags, values, secret) {
  return _chunkRGAQJSYNcjs.serialize.call(void 0, combine(flags, values), flags, secret);
}
async function deserialize2(flags, code, secret) {
  return _chunkRGAQJSYNcjs.deserialize.call(void 0, code, flags, secret);
}
...
async function generatePermutations(flags, filter = null, secret) {
  ...
  return Promise.all(list.map((values) => _chunkRGAQJSYNcjs.serialize.call(void 0, values, flags, secret)));
}
...
function sealHeaders(headers) {
  ...
  if (cached !== void 0)
    return cached;
  const sealed = _chunkJUWXY2C3cjs.HeadersAdapter.seal(headers);
...
function sealCookies(headers) {
  ...
  if (cached !== void 0)
    return cached;
  const sealed = _chunkJUWXY2C3cjs.RequestCookiesAdapter.seal(new (0, _cookies.RequestCookies)(headers));
...
function flag(definition) {
  ...
  const flagImpl = async function flagImpl2(requestOrCode, flagsArrayOrSecret) {
    ...
    const overrides = overridesCookie ? await _chunkJUWXY2C3cjs.decryptOverrides.call(void 0, overridesCookie, store.secret) : void 0;
    if (overrides && hasOwnProperty(overrides, definition.key)) {
      const value2 = overrides[definition.key];
      if (typeof value2 !== "undefined") {
        _chunkJUWXY2C3cjs.reportValue.call(void 0, definition.key, value2);
    ...
    _chunkJUWXY2C3cjs.reportValue.call(void 0, definition.key, value);
    return value;
  };
  ...
  flagImpl.options = _chunkRGAQJSYNcjs.normalizeOptions.call(void 0, definition.options);
...
function getProviderData(flags) {
  const definitions = Object.values(flags).reduce(
    (acc, d) => {
      acc[d.key] = {
        options: _chunkRGAQJSYNcjs.normalizeOptions.call(void 0, d.options),
        origin: d.origin,
      ...
      () => resolve(event, {
        transformPageChunk: async ({ html }) => {
          ...
          if (!store || Object.keys(store.usedFlags).length === 0)
            return html;
          const encryptedFlagValues = await _chunkJUWXY2C3cjs.encryptFlagValues.call(void 0, 
...
async function handleWellKnownFlagsRoute(event, secret, flags) {
  const access = await _chunkJUWXY2C3cjs.verifyAccess.call(void 0, 
  ...
  return Response.json(providerData, {
    headers: { "x-flags-sdk-version": _chunkJUWXY2C3cjs.version }
  });
}
async function encryptFlagValues2(value, secret) {
  return _chunkJUWXY2C3cjs.encryptFlagValues.call(void 0, value, await tryGetSecret(secret));
}
async function decryptFlagValues2(encryptedData, secret) {
  return _chunkJUWXY2C3cjs.decryptFlagValues.call(void 0, encryptedData, await tryGetSecret(secret));
}
async function encryptOverrides2(overrides, secret) {
  return _chunkJUWXY2C3cjs.encryptOverrides.call(void 0, overrides, await tryGetSecret(secret));
}
async function decryptOverrides2(encryptedData, secret) {
  return _chunkJUWXY2C3cjs.decryptOverrides.call(void 0, encryptedData, await tryGetSecret(secret));
}
async function encryptFlagDefinitions2(value, secret) {
  return _chunkJUWXY2C3cjs.encryptFlagDefinitions.call(void 0, value, await tryGetSecret(secret));
}
async function decryptFlagDefinitions2(encryptedData, secret) {
  return _chunkJUWXY2C3cjs.decryptFlagDefinitions.call(void 0, encryptedData, await tryGetSecret(secret));
}
...
function createFlagsDiscoveryEndpoint(getApiData, options) {
  const requestHandler = async (event) => {
    ...
    return _kit.json.call(void 0, apiData, { headers: { "x-flags-sdk-version": _chunkJUWXY2C3cjs.version } });
  };
