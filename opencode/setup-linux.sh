#!/bin/bash
# OpenCode + GatewayZ Setup for Linux
...
if command -v opencode &> /dev/null; then
    ...
    # Method 1: Homebrew (if available)
    ...
    if [ "$INSTALLED" = false ] && command -v npm &> /dev/null; then
        ...
        if sudo npm install -g opencode-ai@latest > /dev/null 2>&1; then
            INSTALLED=true
            print_success "OpenCode installed via npm"
        fi
    fi

    # Method 4: Arch Linux (paru/yay)
    if [ "$INSTALLED" = false ]; then
        if command -v paru &> /dev/null; then
            print_step "Installing via paru (Arch Linux)..."
            if paru -S --noconfirm opencode-bin > /dev/null 2>&1; then
                INSTALLED=true
                print_success "OpenCode installed via paru"
            fi
        elif command -v yay &> /dev/null; then
            print_step "Installing via yay (Arch Linux)..."
            if yay -S --noconfirm opencode-bin > /dev/null 2>&1; then
                INSTALLED=true
                print_success "OpenCode installed via yay"
            fi
        fi
    fi
    ...
    if [ "$INSTALLED" = false ]; then
        ...
        echo -e "${WHITE}  Arch Linux:${NC}  paru -S opencode-bin"
        exit 1
    fi
fi
...
if [ -z "$API_KEY" ]; then
    ...
    echo -e "${YELLOW}═══════════════════════════════════════════${NC}"
    echo ""
    echo -e "${WHITE}To get your API key:${NC}"
    ...
    echo ""
    echo -e "${YELLOW}Press Enter to open the browser, or Ctrl+C to cancel...${NC}"
    read

    # Open browser after user confirms (try common Linux browsers)
