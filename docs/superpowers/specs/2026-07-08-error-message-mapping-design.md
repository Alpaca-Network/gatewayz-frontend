...
- `classifyApiError(status: number, body: unknown, opts?: { retryAfter?: string; context?: string }): AppError` — pure function, given a parsed (or unparseable) JSON body and HTTP status, returns the right `AppError` subtype:
  - `body.error` is an object with `message`/`code`/`type` → use those fields (covers the full envelope and partial pass-through shapes).
  - `body.error` is a string → treat it as the summary, `body.message`/`body.retry_after` as detail (covers the api_keys.py/auth.py 429 variant).
  - `body.detail` is an array of `{type, loc, msg}` → build a field-specific validation message from the first entry (covers the untouched FastAPI 422 default).
  - Otherwise → classify by status code alone: 401→`AUTH_EXPIRED`, 402→`API_PAYMENT_REQUIRED`, 403→`AUTH_INVALID`, 404→`API_NOT_FOUND`, 422→`API_VALIDATION_ERROR`, 429→`API_RATE_LIMITED`, 5xx→`API_SERVER_ERROR`, else `API_ERROR`.
  - If a retry-after value is available (body field or `Retry-After` header), append it to the rate-limit message ("try again in Ns").
  - Raw body/detail text is preserved on `AppError.details` for logging only — never surfaced via `getUserMessage()`.
