...
@pytest.mark.anyio
async def test_happy_path_openrouter(app, happy_patches, payload_basic):
    async with AsyncClient(app=app, base_url="http://test") as ac:
        r = await ac.post("/v1/chat/completions", json=payload_basic)
...
@pytest.mark.anyio
async def test_invalid_api_key(app, monkeypatch, payload_basic):
...
@pytest.mark.anyio
async def test_plan_limit_exceeded_precheck(app, monkeypatch, payload_basic):
...
@pytest.mark.anyio
async def test_rate_limit_exceeded_precheck(app, monkeypatch, payload_basic):
    monkeypatch.setattr(api, "get_user", lambda k: {"id": 1, "credits": 100.0, "environment_tag": "live"})
...
@pytest.mark.anyio
async def test_insufficient_credits_non_trial(app, monkeypatch, payload_basic):
...
@pytest.mark.anyio
async def test_trial_valid_usage_tracked(app, monkeypatch, payload_basic):
...
@pytest.mark.anyio
async def test_trial_expired_403(app, monkeypatch, payload_basic):
    monkeypatch.setattr(api, "get_user", lambda k: {"id": 1, "credits": 0.0, "environment_tag": "live"})
    monkeypatch.setattr(api, "enforce_plan_limits", lambda uid, tok, env: {"allowed": True})
...
@pytest.mark.anyio
async def test_upstream_429_maps_429(app, monkeypatch, payload_basic):
...
@pytest.mark.anyio
async def test_upstream_401_maps_500_in_your_code(app, monkeypatch, payload_basic):
...
@pytest.mark.anyio
async def test_upstream_request_error_maps_503(app, monkeypatch, payload_basic):
    monkeypatch.setattr(api, "get_user", lambda k: {"id": 1, "credits": 100.0, "environment_tag": "live"})
    monkeypatch.setattr(api, "enforce_plan_limits", lambda uid, tok, env: {"allowed": True})
    monkeypatch.setattr(api, "validate_trial_access", lambda k: {"is_valid": True, "is_trial": False})
    monkeypatch.setattr(api, "get_rate_limit_manager", lambda: _RateLimitMgr(True, True))
...
@pytest.mark.anyio
async def test_upstream_timeout_maps_504(app, monkeypatch, payload_basic):
    monkeypatch.setattr(api, "get_user", lambda k: {"id": 1, "credits": 100.0, "environment_tag": "live"})
    monkeypatch.setattr(api, "enforce_plan_limits", lambda uid, tok, env: {"allowed": True})
    monkeypatch.setattr(api, "validate_trial_access", lambda k: {"is_valid": True, "is_trial": False})
    monkeypatch.setattr(api, "get_rate_limit_manager", lambda: _RateLimitMgr(True, True))
...
@pytest.mark.anyio
async def test_saves_chat_history_when_session_id(app, happy_patches, payload_basic):
