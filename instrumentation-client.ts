...
const RATE_LIMIT_CONFIG = {
  ...
  dedupeWindowMs: 60000,         // Don't send same message within 1 minute
  maxBreadcrumbs: 20,            // Allow more breadcrumbs for better debugging
  ...
  maxTransactionsPerMinute: 10,  // Allow 10 transactions per minute
...
function shouldFilterEvent(event: Sentry.ErrorEvent, hint: Sentry.EventHint): boolean {
  ...
  // NOTE: "Pending prompt timed out" errors are now captured (previously filtered)
  // These errors may indicate real user-facing issues that need attention
  ...
  // NOTE: Next.js hydration errors are now captured (previously filtered)
  // These errors may indicate real rendering issues that need attention
...
Sentry.init({
  ...
  // Session replays for debugging errors and user sessions
